    ...
    const priceCalculator = async (_load) => {
        return new Promise( async (resolve, reject) => {
            try {
                ...
                // loop RentalLines
                const processLines = Object.entries(_load.RentalLines).map(async ([key, line]) => {
                    // recipe1 (...more recipes to follow...)
                    await priceCalcRecipe1(line, VAT);
        ...
        return new Promise(  (resolve, reject) => {
            try {
                ...
                if (_line.ItemType === "Sales") {
                    _line.price = _round2Decimals(_line.SalesPrice);
                    _line.totalExcVAT = _round2Decimals(_line.Amount * _line.SalesPrice);
                    _line.totalVAT = _round2Decimals(_line.totalExcVAT * _vat);
                    _line.totalIncVAT = _round2Decimals(_line.totalExcVAT + _line.totalVAT);
                    _line.priceCalcReason = "Fixed sales price";
                    resolve();
                    return;
                }
